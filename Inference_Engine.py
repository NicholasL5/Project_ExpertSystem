...
FILE_PATH = "tugas_2_asp.xlsx"
SOLUTION_VAR = 'support'
premise_table = pd.read_excel(FILE_PATH, sheet_name='Sheet2')
global memory_table
memory_table = {
...
with pd.option_context('display.max_rows', None, 'display.max_columns', None):
    ...
    def update_false():
        ...
        for index, row in premise_table.iterrows():
            if row['Premise Clause Number'][0] in memory_table and not row['Rule Status'][len(row['Rule Status']) - 1] == 'd':
                # kalau jawaban salah
                if not str(row['Premise Clause Number'][1]).strip() == (memory_table[row['Premise Clause Number'][0]]):
    ...
    print(memory_table[SOLUTION_VAR])
